...
train(X, y, target='arousal', drop='valence', model_path, multiprocessing.cpu_count(), args.num_gpus)
...
X = pd.read_csv(input_path, index_col='time')
y = pd.read_csv(output_path, index_col='time')
train(X, y, target='valence', drop='arousal', model_path, multiprocessing.cpu_count(), args.num_gpus)
